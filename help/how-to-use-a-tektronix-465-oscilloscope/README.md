...
FOCUS is for focusing on the signal itself. This might have to be changed as you change your signal.
...
TRACE ROTATION adjusts the trace of your wave to align with your graticule lines. This is also rarely adjusted.

## Screen Units

The lines on the main oscilloscope screen represent graticule markers. These lines are used to measure your wave. Each line represents one division, or “DIV” on most buttons/dials. For instance, when using the channel 1 or channel  2 VOLTS/DIV dials, each vertical deflection will show approximately the highlighted amount of volts PER graticule. If the oscilloscope is properly receiving 2 V into one channel and your dial is set to 1 VOLTS/DIV, then the wave should span across 2 vertical graticules. See the peak-to-peak measurements section for more information on how to measure the full frequency of the wave.

## Channels

Channels 1 and 2 are the main inputs for measuring waves with this oscilloscope. Channel 1 should be the main terminal used, and it is the main terminal used in this guide. When comparing two signals, channel 1 is often used as the main signal to be measured, and channel 2 is the steady reference signal. For the X-Y display, channel 1 or X is the horizontal deflection, and channel 2 or Y is the vertical deflection.
...
The calibrator is the metal rectangle probing out at the bottom right of your oscilloscope. Your first step when receiving your oscilloscope and probes is to calibrate these probes using the calibrator. You should have 1-2 probes for this device. Label your probes for channel 1 and channel 2, and use them accordingly.
...
A vertical and horizontal check will ensure your device is reading waves in a precise manner.
...
1. Step one of the normal sweep display
...
3. Use intensity to find the beam, if not shown, hold down the beam finder and adjust the CH 1 or CH2 VOLTS/DIV until the display is reduced in size. Release the beam finder and adjust the focus for a clearer picture.
...
AUTO mode: The sweep is initiated by the applied trigger signal. If there is no trigger or if the trigger in less than 20 hertz, the sweep free runs and gives a bright reference trace of the input signal. 
...
The triggering knob is labeled with a “Slope Level [- 0 +]”. This indicates the slope that the trigger signal starts on. The “Slope” part of the knob is the knob that clicks to either the negative sign or the plus sign. The “Level” part of the knob indicates where on the wave the trigger will trigger. Keeping this at 0 for basic setups is the most common use, and will keep the trigger line in the center of the horizontal part of the wave.
...
To find the voltage value, first switch the input coupling to GND. If you had a positive voltage, position your line to the bottom reference graticule. If it was a negative voltage, position your line to the top reference graticule. The example below showed a positive voltage so it should be positioned along the bottom reference graticule:
...
In the example below, I am using a square wave on 9k Hz and 1 V. My TIME/DIV setting is set to 0.05 µs. This example is also using the X10 MAG button for better resolution.
...
Measuring the phase difference is similar to measuring the time difference. Instead, the phase difference accounts for the phase degree based on how many graticules a full wave cycle spans.
...
Simply observing the wave like this probably won't give you an accurate measurement, so use the X10 magnification button to get a more accurate display. If using this method, make sure to divide the resulting sweep rate shown by 10. 
...
You can also use this function to use channel 2 frequency as a trigger. To do this, switch the A trigger SOURCE to channel 1, and trigger the LEVEL. Switch the B trigger SOURCE to channel 2 and trigger the LEVEL to where you see a moving wave. 
...
The magnified-sweep-starts-after-delay function allows you to zoom in on your waveform in a more precise way than with just the X10 MAG button. This function allows you to choose the part of your wave you want to zoom in on with the delay time position dial, using the B delay function. 
...
7. To view your wave, set your HORIZ display to either MIX or B OLY'D. Mix allows you to view your main wave with the highlighted portion expanded to the delay you have selected. B DLY'D allows you to view just the highlighted portion.
...
3. Adjust the B LEVEL control so the intensified zone on the trace is visible or stable. If you cannot find the highlighted zone, increase the amplitude or A TIME/DIV. Check that you are not in X10 MAG mode. If these methods don't work, trigger the B sweep externally.
4. Once you have the correct portion highlighted, select B DLY'D or MIX to view the selected display.
...
This section shows you how to use the DELAY TIME POSITION dial as a form of measurement.
